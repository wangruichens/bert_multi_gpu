...
class InputExample(object):
	...
	def __init__(self, guid, text_a, text_b=None, label_reg=None):
...
class DataProcessor(object):
	...
	@classmethod
	def _read_tsv(cls, input_file, quotechar=None, delimiter="\t", do_predict=False):
	...
	def _create_examples(self, lines, set_type):
		...
		for (i, line) in enumerate(lines):
			...
			if set_type == "test":
				...
				val = tokenization.convert_to_unicode(line[2])
			examples.append(
				InputExample(guid = guid, text_a = text_a, label_reg = val))
		return examples
...
def convert_single_example(ex_index, example, max_seq_length,
	...
	if ex_index < 5:
		...
		tf.logging.info("label: %s" % example.label_reg)

	feature = InputFeatures(
		input_ids = input_ids,
		input_mask = input_mask,
		segment_ids = segment_ids,
		label_reg = float(example.label_reg))
	return feature
...
def file_based_convert_examples_to_features(
	...
	for (ex_index, example) in enumerate(examples):
		...
		features["label_reg"] = create_float_feature([feature.label_reg])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
	...
	name_to_features = {
		...
		"label_reg": tf.FixedLenFeature([], tf.float32),
	}
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels, use_one_hot_embeddings, fp16):
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
	...
	def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
		...
		label_reg = features["label_reg"]

		is_training = (mode == tf.estimator.ModeKeys.TRAIN)

		(total_loss, pred_labels) = create_model(
			bert_config, is_training, input_ids, input_mask, segment_ids, label_reg,
		...
		if mode == tf.estimator.ModeKeys.TRAIN:
			...
			def metric_fn(preds, label_ids):
				# Compute MSE
				mse = tf.metrics.mean_squared_error(preds, label_ids)

				return {'MSE': mse}

			eval_metrics = (metric_fn, [pred_labels, label_reg])
			...
			if use_gpu and int(num_gpu_cores) >= 2:
				output_spec = tf.estimator.EstimatorSpec(
					mode = mode,
					predictions = {"probabilities": pred_labels})
			else:
				output_spec = tf.contrib.tpu.TPUEstimatorSpec(
					mode = mode,
					predictions = {"probabilities": pred_labels},
...
def save_for_serving(estimator, serving_dir, seq_length, is_tpu_estimator):
	feature_map = {
		...
		"label_reg": tf.placeholder(tf.float32, shape = [None], name = 'label_reg'),
	}
